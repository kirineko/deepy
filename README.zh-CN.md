...
默认 `deepy` 命令启动稳定的 Rich/prompt-toolkit 终端 UI。可选的 Textual 界面通过下面的命令启动：

```bash
deepy tui
```

TUI 提供可滚动 transcript、实时 thinking 区块、更清晰的工具输出块、slash command 和
`@file` 提示、状态/帮助界面，以及 Deepy 自有的 diff view。它仍是实验功能，后续版本可能调整交互和外观。

![Deepy Textual TUI](asset/deepy-tui.webp)

`/status` 会在一个面板中汇总 session/project usage、context window 和 DeepSeek 余额。
退出 TUI 时会输出和稳定 UI 一致的 compact session summary。
...
| 稳定 UI 与实验 TUI | [docs/deepy-ui-and-tui.zh-CN.md](docs/deepy-ui-and-tui.zh-CN.md) | [docs/deepy-ui-and-tui.md](docs/deepy-ui-and-tui.md) |
