...
本文说明 Deepy 的两个终端界面，并记录当前功能对齐情况。

## 界面定位

- **Deepy UI**：默认稳定界面，运行 `deepy` 启动。
...
Deepy TUI 会把 transcript、prompt、状态行和快捷键提示放在同一个 Textual 界面中。

![Deepy Textual TUI](../asset/deepy-tui.webp)

`/status` 会按需显示用量、上下文窗口压力和 DeepSeek 余额。

![Deepy TUI status 面板](../asset/tui-status.webp)
...
- `shell`、`read`、`todo_write`、Web、MCP、`load_skill` 和后台任务工具有专门显示效果。
