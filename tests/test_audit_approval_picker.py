...
from deepy.ui.audit_approval_picker import AUDIT_APPROVAL_REJECT
from deepy.ui.audit_approval_picker import AUDIT_APPROVAL_TOGGLE_PREVIEW
from deepy.ui.audit_approval_picker import AuditApprovalPicker
...
def test_audit_approval_picker_scrolls_expanded_panel_with_vim_keys(monkeypatch):
    monkeypatch.setattr("deepy.ui.audit_approval_picker.shutil.get_terminal_size", lambda fallback: os.terminal_size((80, 12)))
    picker = AuditApprovalPicker(
        can_toggle_preview=True,
        expanded=True,
        panel_text_factory=lambda _expanded: "\n".join(f"line {index}" for index in range(20)),
