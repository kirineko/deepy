...
import deepy.ui.classic.terminal as terminal
...
from deepy.ui.classic.pickers.skill_picker import SkillMenuAction
from deepy.ui.classic.terminal import _collect_pending_question_response
from deepy.ui.classic.terminal import _build_status_footer
from deepy.ui.classic.terminal import _format_context_footer
from deepy.ui.classic.terminal import _handle_slash_command
from deepy.ui.classic.terminal import _print_assistant_output
from deepy.ui.classic.terminal import _print_stream_event
from deepy.ui.classic.terminal import _print_user_input
from deepy.ui.classic.terminal import _print_usage_footer
from deepy.ui.classic.terminal import _format_duration_ms
from deepy.ui.classic.terminal import _format_stream_token_count_short
from deepy.ui.classic.terminal import _format_token_count_short
from deepy.ui.classic.terminal import _tool_output_text
from deepy.ui.classic.terminal import _run_once_with_status
from deepy.ui.classic.terminal import _working_status_text
...
import deepy.ui.classic.terminal
"""
...
def test_skills_slash_command_shows_management_menu(tmp_path):
    ...
    monkeypatch.setattr(
        "deepy.ui.classic.commands.skill_commands.search_market_skills",
        lambda query: (_ for _ in ()).throw(AssertionError("market should load inside picker")),
    )
    monkeypatch.setattr(
        "deepy.ui.classic.commands.skill_commands._build_installed_skill_views",
        lambda project_root: ["installed-demo"],
    )
    ...
    monkeypatch.setattr("deepy.ui.classic.commands.skill_commands.pick_skill_menu_action", fake_pick)
    try:
...
def test_skills_menu_install_action_asks_for_scope_and_runs_market_install(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("deepy.ui.classic.commands.skill_commands.search_market_skills", lambda query: ["market-demo"])
    monkeypatch.setattr("deepy.ui.classic.commands.skill_commands.list_installed_skills", lambda: [])
    monkeypatch.setattr(
        "deepy.ui.classic.commands.skill_commands.pick_skill_menu_action",
        lambda market, installed, market_loader=None: next(actions),
    )
    monkeypatch.setattr(
        "deepy.ui.classic.commands.skill_commands.pick_skill_install_scope",
    ...
    monkeypatch.setattr("deepy.ui.classic.commands.skill_commands.install_market_skill", fake_install_market_skill)
...
def test_skills_menu_show_installed_skill_opens_detail_view(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("deepy.ui.classic.commands.skill_commands.show_skill_detail_view", shown.append)
...
def test_print_stream_event_passes_console_width_to_diff_preview(monkeypatch):
    console = Console(record=True, width=72, height=24)
...
def test_status_slash_command_prints_status(tmp_path, monkeypatch):
    console = Console(record=True, width=200, height=24)
    calls = 0
...
def test_theme_slash_command_uses_keyboard_picker_when_no_input_func(tmp_path, monkeypatch):
    ...
    config.write_text('[ui]\ntheme = "light"\n', encoding="utf-8")
    console = Console(record=True)
    monkeypatch.setattr("deepy.ui.classic.commands.model_commands.pick_theme", lambda current: "dark")
...
def test_terminal_approval_panel_keeps_outside_project_path_explicit(tmp_path):
    console = Console(record=True, width=120, height=24)
...
def test_runtime_status_line_tail_truncates_long_local_command():
    command = "uv run pytest tests/ui/classic/test_terminal.py::test_runtime_status --very-long-option"
...
def test_run_interactive_shows_fast_version_update_in_welcome(tmp_path, monkeypatch):
    console = Console(record=True, width=160, height=24)
